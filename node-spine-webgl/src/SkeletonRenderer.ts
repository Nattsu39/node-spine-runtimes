...
export class SkeletonRenderer {
	...
	draw(
		...
		skeleton: Skeleton,
		slotRangeStart: number = -1,
		slotRangeEnd: number = -1,
	) {
		...
		let uvs: ArrayLike<number> | null = null;
		let triangles: Array<number> | null = null;
		let drawOrder = skeleton.drawOrder;
		let attachmentColor: Color | null = null;
			...
			if (texture != null) {
				...
				if (clipper.isClipping()) {
					...
					if (this.vertexEffect != null) {
						...
						if (!twoColorTint) {
							for (
								let v = 0, u = 0, n = renderable.numFloats;
								v < n;
								v += vertexSize, u += 2
							) {
								tempPos.x = verts[v];
						...
						if (!twoColorTint) {
							for (
								let v = 2, u = 0, n = renderable.numFloats;
								v < n;
								v += vertexSize, u += 2
							) {
					...
					let view = (renderable.vertices as Float32Array).subarray(
						0,
						renderable.numFloats,
					);
